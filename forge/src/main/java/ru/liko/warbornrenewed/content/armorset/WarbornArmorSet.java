...
public final class WarbornArmorSet {
    ...
    private WarbornArmorSet(String id,
        ...
        this.id = id;
        EnumMap<ArmorItem.Type, List<RegistryObject<WarbornArmorItem>>> byTypeCopy = new EnumMap<>(
    ...
    static WarbornArmorSet register(DeferredRegister<Item> items, Builder builder) {
        EnumMap<ArmorItem.Type, List<RegistryObject<WarbornArmorItem>>> registrations = new EnumMap<>(
                ArmorItem.Type.class);
        List<RegistryObject<WarbornArmorItem>> all = new ArrayList<>();
        builder.definitions.forEach((type, definitions) -> {
            List<RegistryObject<WarbornArmorItem>> entries = registrations.computeIfAbsent(type,
                    key -> new ArrayList<>());
            for (ArmorPieceDefinition definition : definitions) {
                RegistryObject<WarbornArmorItem> entry = items.register(definition.registryName(),
